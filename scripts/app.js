...
// the verse band a type occupies, from its own floor and its neighbours' — one home: the key,
// the index table and the draft generator all read it. The ends are open: "≥ 40", "≤ 2".
function wayBandOf(types, typeKey) {
  const i = types.findIndex(t => t.key === typeKey), lo = types[i].minVerses;
  if (i === 0) return `≥ ${lo}`;
  const hi = types[i - 1].minVerses - 1;
  return i === types.length - 1 ? `≤ ${hi}` : `${lo}–${hi}`;
}
...
const wayBand = typeKey => wayBandOf(WAYDATA.types, typeKey);

/* Every caveat the key and the index table state, in one place so the two cannot drift. Who
   supplied what matters: PaulDz gave the six types and the verse lists; the blocks are our join
   of those verses onto the spreadsheet's pericopes; the ORDER of the types and their verse
   bands are our proposal. */
const WAY_TEXT = {
  statusPrefix: "Each theme's type (our order and verse bands): ",
  provenance: "Verses are PaulDz's lists; blocks are the pericopes those verses fall in; each way's type is our proposal, derived from its verse count.",
  greek: "Greek ≈ (est.) spreads each pericope's word count evenly over its verses; it is not a count of the theme's own verses.",
  schematic: "Each line joins its theme's blocks in verse order; between two blocks it carries none of the theme's verses, and its route there is schematic.",
  discs: "A numbered disc marks a block holding at least one of that way's verses; the whole pericope is lit. Grey blocks hold none of the shown ways' verses.",
  wall: "Theme ways pass beneath the city wall; they make no gates.",
};
...
function wayKeyRow(parent, k, n, scale) {
  ...
  const named = t.blocks.length <= 2 ? ` (${t.blocks.map(blockName).join(", ")})` : "";
  dom("div", "wk-facts", `${plural(t.verses, "verse")} → ${ty.label} (band ${wayBand(t.way)}) · ` +
    `${plural(t.blocks.length, "block")}${named} · Greek ≈ ${fmt(t.greek)} (est.)${refs}`, row);
  // blocks this way shares with each other shown way, computed from the two block lists
  WAYS.shown.forEach((m, j) => {
    if (m === k) return;
    const other = new Set(WAY_THEME.get(m).blocks), both = t.blocks.filter(b => other.has(b));
    if (!both.length) return;
    const what = both.length <= 2 ? both.map(b => `${blockName(b)} (${(byId[b] || {}).ref || b})`).join(" and ") : plural(both.length, "block");
  ...
  if (pinches.length) {
    ...
    pinches.forEach((q, i) => {
      const sp = dom("span", null, `${i ? " · " : ""}${blockName(q.a)} & ${blockName(q.b)}`, note);
...
function renderKey() {
  ...
  const st = dom("p", "wk-head-status", WAY_TEXT.statusPrefix, key);
  dom("span", "wk-status", WAYDATA.status, st);
  WAYS.shown.forEach((k, i) => wayKeyRow(key, k, i + 1, scale));
  const foot = dom("div", "wk-foot", null, key);
  for (const k of ["provenance", "schematic", "discs", "greek", "wall"]) dom("p", `wk-${k}`, WAY_TEXT[k], foot);
...
function renderWaysIndex(iv) {
  ...
  const cap = dom("caption", null, WAY_TEXT.statusPrefix, table);
  dom("b", null, WAYDATA.status, cap);
  cap.append(". " + WAY_TEXT.provenance + " " + WAY_TEXT.greek);

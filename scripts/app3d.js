...
const PAL = {
  light: {
    skyTop: "#cfe3ef", skyBot: "#f2ead8", ground: "#e9e2cf", groundEdge: "#d8cfb6",
    district: "rgba(120,112,96,0.10)", sea: "#bcd9e4", seaEdge: "#8fb6c4",
    road: "#efe6d1", roadEdge: "#c9bfa4", wall: "#7d7566", wallTop: "#948b7a",
  ...
  dark: {
    ...
    road: "#39352b", roadEdge: "#4a463c", wall: "#5c5648", wallTop: "#6d6757",
...
function viewMatrix() {
  ...
  if (mode === "orbit") {
    ...
    const ahead = wayAt(Math.min(1, cam.walkT + 0.012));
...
function render() {
  ...
  if (document.getElementById("ck3-labels").checked) {
    ...
    if (mode === "walk" || cam.dist < 520) {
      HOODS.forEach(h => drawLabel(h.x, h.h + 9, h.y, h.short, 10, P().ink));
      OBELISKS.forEach(o => drawLabel(o.x, o.hgt + 8, o.y, "✦ " + o.s.label, 9.5, P().goldDark));
      drawLabel(AX.x, AX.h + 9, AX.y, AX.short, 10, P().ink);
    }
